...
//F4 = 62
//F12 = 88
static std::vector<key_state> libinput_key_states;
...
static input_thread it;
static std::unique_ptr<input_thread, std::function<void(input_thread*)>> holder;
...
uint8_t get_linux_key(uint16_t x11_key) {
...
static void event_thread(input_thread* it)
{
    ...
    while (!it->quit) {
        while ((event = libinput_get_event(li)) != NULL) {
            ...
            if (LIBINPUT_EVENT_KEYBOARD_KEY == event_type) {
                ...
                for (auto& k : libinput_key_states) {
                    if (k.key == key)
...
void start_input(const overlay_params& params)
{
    ...
    libinput_key_states.push_back({get_linux_key(params.toggle_hud), false, 0});
    libinput_key_states.push_back({get_linux_key(params.toggle_logging), false, 0});
    libinput_key_states.push_back({get_linux_key(params.reload_cfg), false, 0});

    it.thread = std::thread(event_thread, &it);
    holder = {
        &it, [](input_thread* i){
            i->quit = true;
...
bool input_key_is_pressed(unsigned long ks)
{
    auto linux = get_linux_key(ks);
    if (!linux)
        return false;

    for (auto&k : libinput_key_states)
        if (k.key == linux && k.pressed)
            return true;
    return false;
}
